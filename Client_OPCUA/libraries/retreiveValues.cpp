...
RetreiveValues::~RetreiveValues() { UA_Client_delete(client_); }
...
bool RetreiveValues::connect(const std::string &serverUrl) {
  ...
  if (retval != UA_STATUSCODE_GOOD) {
    std::cerr << "Failed to connect to server. StatusCode: 0x" << std::hex
              << retval << std::endl;
    return false;
  }
  return true;
...
bool RetreiveValues::browseNode(const UA_NodeId &nodeId,
                                UA_BrowseResponse &bResp) {
  return browseRequestBuilder(nodeId, bResp);
}

bool RetreiveValues::findFolder(UA_BrowseResponse &bResp,
                                const std::string &folderName,
                                UA_NodeId &foundId,
                                UA_ReferenceDescription **foundRef) {
...
bool RetreiveValues::readTemperature(const UA_NodeId &tempSensorId,
                                     UA_Int32 &temperature) {
  UA_Variant value;
  UA_Variant_init(&value);
  UA_StatusCode retval =
      UA_Client_readValueAttribute(client_, tempSensorId, &value);
  ...
  if (retval == UA_STATUSCODE_GOOD &&
    ...
    std::cerr << "Failed to read Temperature value. StatusCode: 0x" << std::hex
              << retval << std::endl;
...
bool RetreiveValues::browseRequestBuilder(UA_NodeId nodeId,
                                          UA_BrowseResponse &bResp) {
  UA_LOG_INFO(
      UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
      "Browsing node: namespaceIndex=%u, identifierType=%d, identifier=%.*s",
  ...
  if (bResp.responseHeader.serviceResult != UA_STATUSCODE_GOOD) {
    UA_LOG_ERROR(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                 "Browse request failed. StatusCode: 0x%08x",
...
bool RetreiveValues::ObjectFinder(UA_BrowseResponse &bResp,
                                  UA_String ParentFolder, UA_NodeId *foundId,
                                  UA_ReferenceDescription **foundRef) {
      ...
      if (UA_String_equal(&currentRef->browseName.name, &ParentFolder)) {
        ...
        *foundRef = currentRef;
        UA_LOG_INFO(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                    "Found folder: %.*s", (int)ParentFolder.length,
                    ParentFolder.data);
  ...
  UA_LOG_WARNING(UA_Log_Stdout, UA_LOGCATEGORY_USERLAND,
                 "Folder not found: %.*s", (int)ParentFolder.length,
                 ParentFolder.data);
  return false;
}
